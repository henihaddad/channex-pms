...
**Status:** `accepted` — implemented in M5 (v0.4, 2026-09-02); see §17.8 for what shipped and what is deferred. Added 2026-08-21 with the STR segment decision.
...
## 17.8 Implementation notes (M5, v0.4)

- **Engine.** `computeStatement` in `core/owners` is pure over an explicit input (segments,
  nights, approved expenses, owner-stay nights, adjustments, released hold-back). Lines are
  sorted by a total order, negations never produce `-0`, and the same input serialises to the
  same bytes (OWN-1). Property tests cover: lines sum to net, only period nights appear,
  consecutive periods partition every night, exact allocation of booking-level amounts.
- **Nights.** Revenue is attributed from `booking_room_day` (STMT-3). A booking's OTA commission
  and OTA-withheld taxes are allocated over all its nights by amount, so a night's share is the
  same whichever period bills it; the cleaning fee (a service named "clean…") sits on the first
  night. Commission is `actual` when the booking reports it; otherwise an `estimate` at the
  channel's usual rate (Booking.com 15 %, Airbnb 3 %, Expedia 18 %), labelled on the line and
  counted in the review flags.
- **Basis.** `net_of_tax` uses the taxes the OTA withheld; inclusive taxes are not yet separated.
- **Models.** `fixed_fee` and `guaranteed_rent` are prorated by the segment's share of the period's
  days; under guaranteed rent the fee line is whatever keeps the owner at the rent, positive when
  revenue falls short. `tiered` applies the band the basis falls in to the whole basis.
- **Once and only once.** `owner_statement_night` records every billed night with a unique index
  on (room, date); a draft is regenerated by rewriting its rows; a sent statement is frozen by
  triggers (INV-13) and restatements come from comparing that ledger with the current nights
  (STMT-4), keyed so each restatement is applied once.
- **Schedule.** `statements.sweep` (daily 04:00) drafts the period that ended before today for every
  active agreement (monthly or fortnightly) and refreshes drafts whose inputs changed;
  `statements.autosend` sends approved statements after `statement_auto_send_days` (default 7)
  unless `statement_auto_send` is `off` in the org settings.
- **PDF.** Rendered by a dependency-free writer in `packages/jobs` (Helvetica/Courier, WinAnsi;
  other scripts fall back to `?`) and stored inline in `pdf_ref`; object storage comes with the
  storage port. STMT-6 currency conversion is not implemented: the agreement currency is the
  property currency.
- **Payouts.** `PayoutProvider` port with `FakePayoutProvider` and a Stripe Connect adapter over
  the REST API (unit-tested with a fake transport; no live account). `payout:execute` is step-up
  gated by the matrix; above `payout_four_eyes_minor` (default 5,000.00) the payout waits for a
  second person. Manual bank transfers record a reference and mark the statement paid.
- **Portal.** The `owner` role on the owner's own group (spec 02 §2.1), magic-link sign-in, every
  read scoped through `owner.user_id` (OWN-3). Owner stays go through `unit_block` and the ARI
  pipeline (PORT-3); disputes open an inbox thread tagged `dispute` and set `dispute_state`
  (ADR-0003). Deferred: consolidated multi-property statements (STMT-7), currency conversion
  (STMT-6), expense CSV import, agreement templates, the owner acquisition report (§17.6).
