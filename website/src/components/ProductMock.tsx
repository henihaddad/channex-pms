...
const rows: { name: string; cells: (null | { span: number; ch: string; tone: string })[] }[] = [
  {
    name: "Bourguiba 12",
    cells: [{ span: 3, ch: "Airbnb", tone: "bg-rose-400/80" }, null, { span: 2, ch: "Booking.com", tone: "bg-sky-400/80" }, null],
  },
  {
    name: "La Marsa 4B",
    cells: [null, { span: 4, ch: "Direct", tone: "bg-mint/80" }, null, { span: 1, ch: "Expedia", tone: "bg-amber/80" }],
  },
  {
    name: "Sidi Bou Said Loft",
    cells: [{ span: 2, ch: "Booking.com", tone: "bg-sky-400/80" }, null, { span: 3, ch: "Airbnb", tone: "bg-rose-400/80" }, null],
  },
];
    ...
    <div className="grid gap-4" aria-hidden="true">
      <div className="rounded-2xl border border-line bg-panel p-4 shadow-2xl shadow-black/40">
        <div className="flex items-center justify-between text-xs text-fg-muted">
          ...
          <span className="rounded-full bg-mint/15 px-2 py-0.5 font-medium text-mint">All 142 listings in sync</span>
        </div>
          ...
          {rows.map((r) => {
            ...
            for (const c of r.cells) {
              if (c) {
                ...
                cells.push(<div key={`${r.name}-${i}`} className="mx-0.5 rounded-md border border-dashed border-line py-1.5" />);
                i += 1;
              }
            }
            return (
      ...
      <div className="flex flex-col justify-between rounded-2xl border border-line bg-panel-2 p-4 text-xs shadow-xl">
        ...
        <ul className="mt-3 space-y-1.5 text-fg-muted">
          <li className="flex justify-between"><span>La Marsa 4B</span><span className="text-fg">Amira · 11:00</span></li>
          <li className="flex justify-between"><span>Sidi Bou Said Loft</span><span className="text-fg">Karim · 12:30</span></li>
          <li className="flex justify-between"><span>Bourguiba 12</span><span className="text-fg">Amira · 14:00</span></li>
        </ul>
        <p className="mt-3 rounded-md bg-sky/10 px-2 py-1 text-[11px] font-medium text-sky">Access codes sent to guests.</p>
      </div>
      </div>
    </div>
  );
