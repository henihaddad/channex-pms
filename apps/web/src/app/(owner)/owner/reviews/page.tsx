...
export default async function OwnerReviews() {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-slate-600">{t("reviewsHint")}</p>
      <Card className="text-sm">
        {rows.length === 0 ? <p className="text-xs text-slate-500">{t("noReviews")}</p> : null}
        {rows.map((r) => (
          <p key={r.id} className="border-t border-slate-100 py-1 text-xs">
